...
test_path=${tazer_path}/tests/composable_server_test/tazer_cp_read_test_client.sh
...
for shared_mem in ${CACHE_SIZES[@]}; do
    for burst_buffer in ${CACHE_SIZES[@]}; do
        for bounded_filelock in ${CACHE_SIZES[@]}; do
            ...
            srun ${test_path} ${node_workspace} ${data_path} ${tazer_path} ${build_dir} ${server_addr} ${server_port} ${test_id} ${shared_mem} ${burst_buffer} ${bounded_filelock} ${ref_time}

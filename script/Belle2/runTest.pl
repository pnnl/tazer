...
my $path = "/people/mutl832/tazer-paper/build";
    ...
    foreach my $file (@clientFiles) {
        if($file =~ /meta/) {
            ...
            while ( my $line = <$fh> ) {
                ...
                elsif (index( $line, 'port') > -1) {
                    $newText .= "port=$port\n";
                }
                elsif(index( $line, '/home/frie869/ippd_data/') > -1){
                    $line =~ s/\/home\/frie869\/ippd_data\//\/files0\/frie869\/ippd_data\//;
                    $newText .= $line
                }
                else {
                    $newText .= $line; 
